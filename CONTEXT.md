...
与某位 Referrer(邮箱 + 可选显示名)绑定的不透明随机串,用于生成推荐链接。一条有效链可服务多名朋友。带过期时间;创建它的 Referral Ops Account 可作废、续期、重新生成。不挂本系统 Application/Expert。同一 Referral Ops Account 下同一 Referrer 邮箱至多一条 ACTIVE;不同账号可各有一条。
...
**Ops(运营方)**:
平台内部运营人员这一角色。管理邀请批次、专家档案、推荐人与推荐 token。
_Avoid_: admin, staff, operator

**Ops Account(运营账号)**:
专家档案与邀请生成页共用的单一登录身份(用户名 + 密码)。由 `OPS_EXPERT_FILES_USERNAME` 配置,与推荐页会话相互独立。
_Avoid_: operator, admin account, ops user

**Referral Ops Account(推荐运营账号)**:
推荐链接生成页的登录身份(用户名 + 密码)。由部署配置预置允许的用户名名单,共用初始密码,首次登录后各自改密。与专家档案/邀请的 Ops Account 分属不同会话与账号表。推荐 token 的创建归属落在 Referral Ops Account 上;每个账号只能看见并操作自己创建的推荐链接,无跨账号特权视图。
_Avoid_: Ops Account, referral admin
