...
export function ReferralOpsPanel() {
  ...
  async function generate(entries: Entry[]) {
    ...
    try {
      ...
      const created =
        (payload.results?.length ?? 0) - skipped;
      await load();
      toast.success(
        skipped > 0
  ...
  async function mutate(tokenId: string, action: "DISABLE" | "RENEW" | "REGENERATE") {
    setBusy(true);
    try {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-heading text-3xl font-semibold">推荐链接</h1>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" nativeButton={false} render={<a href="/ops/invitations" />}>
            邀请生成
          </Button>
          <Button variant="outline" size="sm" nativeButton={false} render={<a href="/ops/expert-files" />}>
            专家档案
          </Button>
        </div>
      </header>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  {item.referrerDisplayName ? (
                    <p className="text-muted-foreground">{item.referrerEmail}</p>
                  ) : null}
              ...
              {expandedId === item.id ? (
                <div className="bg-muted/40 rounded-md p-3">
                  {(downstream[item.id] ?? []).length === 0 ? (
                    <p className="text-muted-foreground text-xs">暂无注册账号。</p>
                  ) : (
